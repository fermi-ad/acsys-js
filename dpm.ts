...
import {
	DPM_PROTO, DPM_request_ServiceDiscovery, DPM_request_OpenList, DPM_request_StartList, DPM_request_ClearList, DPM_request_StopList, DPM_reply_StartList, DPM_request_AddToList, DPM_reply_ServiceDiscovery, DPM_reply_OpenList, DPM_reply_ListStatus, DPM_reply_Status, DPM_reply_DeviceInfo, DPM_reply_Scalar, DPM_reply_ScalarArray, DPM_reply_Raw, DPM_reply_Text, DPM_reply_TextArray, DPM_reply_AnalogAlarm, DPM_reply_DigitalAlarm, DPM_reply_BasicStatus, DPM_Replies
...
export class DPM {
	...
	private findDPM(server?: string): () => Promise<string> {
		if (server !== undefined) {
			if (server.includes('@')) {
				...
				return async () => Promise.resolve(`DPMD@${server}`);
		} else
			return async () => {
				...
				if (msg !== undefined) {
					if (msg instanceof DPM_reply_OpenList) {
						...
						if (errCallback !== undefined)
							...
							console.info(`DPM: error status ${s} for ${this.reqs[msg.ref_id].request}`);
						}
					} else if (msg instanceof DPM_reply_Scalar ||
	...
	constructor(server?: string, shConn?: ACNET) {
		...
		Promise.resolve(server)
			.then(server => this.connectionManager(server));
	}
	...
	async addRequest(req: string, cb: (o: any, i: DPM_reply_DeviceInfo) => boolean,
		ecb?: (e: any) => void) {
